...
use jni::{
    ...
    objects::{Global, JClass, JObject, JObjectArray, JString},
    refs::Reference,
    sys::{jbyte, jobjectArray, jstring},
};
...
fn ensure_permission(
    ...
    let granted = env.call_static_method(
        &activity_class,
        jni_str!("requestUsbPermissionBlocking"),
        jni_sig!("(Landroid/hardware/usb/UsbDevice;)Z"),
        &[JValue::Object(device)],
    );
    check_exception(env).context("Android USB permission request failed")?;
    let granted = granted?.z()?;
    if granted {
        info!("Android USB permission granted");
        Ok(())
    } else {
        bail!("Android USB permission denied or timed out")
    }
}
    ...
    let message = env
        ...
        .and_then(|object| {
            if object.is_null() {
                ...
                let message = unsafe { JString::from_raw(env, object.into_raw() as jstring) };
