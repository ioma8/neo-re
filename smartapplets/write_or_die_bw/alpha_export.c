...
static uint32_t AppendThroughAlphaWordBuffers(const WodEditor_t* editor, uint32_t slot) {
    ...
    for(uint32_t name = 0x1000u; name < 0x2400u; name++) {
        ...
        if(used_long != used_mirror) {
            continue;
        }
        uint32_t capacity = capacity_long;
        if(capacity != 512u && capacity != 9216u) {
            capacity = capacity_mirror;
        }
        if(used_long > capacity || (capacity != 512u && capacity != 9216u)) {
            continue;
        }
        candidates++;
        best_record = record;
    ...
    if(best_record != 0 && AppendToAlphaWordBuffer(&memory[best_record], best_pointer, (uint16_t)best_used, (uint16_t)best_capacity, editor)) {
        return 1;
    }
...
uint32_t alpha_export_append_session(const WodEditor_t* editor, uint32_t slot) {
    ...
    if(!PreselectAlphaWordFile(applet, slot)) {
        return 3;
    }
    uint32_t backing_slot = slot == 1u ? 8u : slot - 1u;
    uint32_t append_status = AppendThroughAlphaWordBuffers(editor, backing_slot);
    if(append_status == 0) {
        return 3;
    }
