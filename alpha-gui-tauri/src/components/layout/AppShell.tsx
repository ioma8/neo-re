  ...
  return (
    <div className="min-h-screen bg-background text-on-surface">
      ...
      <main className="min-h-screen pb-24 lg:ml-64 lg:pb-0">
        <header className="sticky top-0 z-20 border-b border-gray-200 bg-white px-4 py-2 font-sans text-sm font-medium antialiased lg:hidden">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 grid h-16 grid-cols-4 border-t border-gray-200 bg-white px-4 text-[10px] font-medium uppercase tracking-wider lg:hidden">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            onClick={() => onTabChange(tab.key)}
